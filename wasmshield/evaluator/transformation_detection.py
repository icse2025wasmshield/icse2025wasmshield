...
class TransformsEvaluator:
    ...
    def __init__(
        self,
        device,
        preprocessor=wasmshield.preprocessing.preprocess_image,
        n_train=1000,
        n_test=1000,
    ):
        ...
        try:
            ...
            for s_type, s_test in [
                ...
                for s, f_pairs, posi_pairs, nega_pairs, n in [
                    (
                        s_test,
                        self.test_new_pairs_f,
                        self.posi_test_new_pairs,
                        self.nega_test_new_pairs,
                        n_test
                    ),
                ]:
        ...
        try:
            ...
            for s_type, s_train, s_test in [
                ...
                # (
                #     transform_types.wasm_bench_mutation_levels,
                #     ,
                #     ,
                # ),
            ]:
                for s, f_pairs, posi_pairs, nega_pairs, n in [
                    (
                        s_train,
                        self.train_pairs_f,
                        self.posi_train_pairs,
                        self.nega_train_pairs,
                        n_train,
                    ),
                    (
                        s_test,
                        self.test_pairs_f,
                        self.posi_test_pairs,
                        self.nega_test_pairs,
                        n_test
                    ),
                ]:
                    self.transform_processor(n, s, f_pairs, posi_pairs, nega_pairs, s_type)
    ...
    def get_vectors(self, files, model):
        input = torch.tensor(
            np.array([
                self.preprocessor(f) 
                for f in files
            ]), device=device, dtype=torch.float32).to(device)
        output = model.backbone(input).cpu().numpy()
        return output

    def get_pairs_vectors(self, pair, vector_getter, chunksize=512, **kwargs):
        ...
        for t, li in tqdm.tqdm(list(pair.items())):
            ...
            for chunk in divide_chunks(li,chunksize):
                k[t].extend(vector_getter(chunk, **kwargs))
        return k
    
    def get_X_y(self, **kwargs):

        train_pairs_f_v,posi_train_pairs_v,nega_train_pairs_v = (
            self.get_pairs_vectors(self.train_pairs_f, **kwargs),
            self.get_pairs_vectors(self.posi_train_pairs, **kwargs),
            self.get_pairs_vectors(self.nega_train_pairs, **kwargs),
        )
        
        test_pairs_f_v,posi_test_pairs_v,nega_test_pairs_v = (
            self.get_pairs_vectors(self.test_pairs_f, **kwargs),
            self.get_pairs_vectors(self.posi_test_pairs, **kwargs),
            self.get_pairs_vectors(self.nega_test_pairs, **kwargs),
        )

        test_new_pairs_f_v,posi_test_new_pairs_v,nega_test_new_pairs_v = (
            self.get_pairs_vectors(self.test_new_pairs_f, **kwargs),
            self.get_pairs_vectors(self.posi_test_new_pairs, **kwargs),
            self.get_pairs_vectors(self.nega_test_new_pairs, **kwargs),
        )
